...
public class InventoryUI : MonoBehaviour
{
    ...
    public GameObject btnShowDiary;
    ...
    private void Update()
    {
        ...
        {
            if (InventorySystem.instance != null && !InventorySystem.instance.IsInventoryUnlocked())
                return;
    ...
    {
        if (smallSlotsContent == null || smallSlotPrefab == null || InventorySystem.instance == null) return;
        foreach (Transform child in smallSlotsContent) Destroy(child.gameObject);
        foreach (InventorySlot slot in InventorySystem.instance.smallSlots)
        {
            if (slot.itemData != null) CreateSlotUI(slot, smallSlotPrefab, smallSlotsContent);
        }
    }

    private void UpdateWeaponSlotsDisplay()
    {
        if (weaponSlotsContent == null || InventorySystem.instance == null) return;

        while (InventorySystem.instance.weaponSlots.Count < InventorySystem.instance.maxWeaponSlots)
            InventorySystem.instance.weaponSlots.Add(new InventorySlot(null, 0));
        ...
        for (int i = 0; i < weaponSlotsContent.childCount; i++)
        {
            ...
            if (i < InventorySystem.instance.maxWeaponSlots)
            {
                InventorySlot slot = InventorySystem.instance.weaponSlots[i];
    ...
    private void UpdateWeaponSlotUI(GameObject slotUI, InventorySlot slot, int slotIndex)
    {
        ...
        if (itemIcon == null && slotUI.transform.childCount > 0)
            itemIcon = slotUI.transform.GetChild(0).GetComponent<Image>();
        if (itemIcon == null)
            itemIcon = slotUI.GetComponent<Image>();
    ...
    private void CreateSlotUI(InventorySlot slot, GameObject prefab, Transform parent)
    {
